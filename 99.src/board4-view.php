...
$sql = "SELECT ifnull(max(pp.wr_id), 0) as prev_id FROM (SELECT temp.wr_id FROM g5_write_gallery temp WHERE temp.wr_id ) pp WHERE pp.wr_id < $wr_id";
$result = mysqli_query($conn, $sql) or exit(mysqli_error($conn));
$prev_data = mysqli_fetch_array($result);
$result->free();
$prevId = intVal($prev_data['prev_id']);

$sql = "SELECT ifnull(min(nn.wr_id), 0) as next_id FROM (SELECT temp.wr_id FROM g5_write_gallery temp WHERE temp.wr_id ) nn WHERE nn.wr_id > $wr_id";
$result = mysqli_query($conn, $sql) or exit(mysqli_error($conn));
...
$img_data = array();
if ($isFile > 0) {
    $sql = "SELECT bf_source, bf_file, bf_download, bf_type FROM g5_board_file where bo_table = 'gallery' AND  wr_id = $wr_id";
    ...
    <div class="container_inner">
        <div class="content_section">
            <div class="board_list_w">
                <h3 class="content_title_board">갤러리</h3>
